...
func TestBMW157C165Checksum(t *testing.T) {
	file, err := os.Open("./firmwares/BMW/157C165_ori.bin")
	defer file.Close()
	if err != nil {
		t.Error(err)
		return
	...
	if err = BMW157C165Validate(buf); err != nil {
	...
	if sum != 0x6A57 {
		t.Errorf("Checksum failed, expected: %X got: %X", 0x6A57, sum)
	}
}
...
func TestBMW484C582Checksum(t *testing.T) {
	file, err := os.Open("./firmwares/BMW/484C582_ori.bin")
	defer file.Close()
	if err != nil {
		t.Error(err)
		return
	...
	if err = BMW484C582Validate(buf); err != nil {
		t.Errorf("Invalid test binary file")
		return
	}

	sum, _ := BMW484C582Checksum(buf, false)
	if sum != 0xB011 {
		t.Errorf("Checksum failed, expected: %X got: %X", 0xB011, sum)
	}
}

func TestBMW590C597Checksum(t *testing.T) {
	file, err := os.Open("./firmwares/BMW/590C597_ori.bin")
	defer file.Close()
	if err != nil {
		t.Error(err)
		return
	...
	if err = BMW590C597Validate(buf); err != nil {
		t.Errorf("Invalid test binary file")
		return
	}
...
func TestBMW990C070Checksum(t *testing.T) {
	file, err := os.Open("./firmwares/BMW/990C070_ori.bin")
	defer file.Close()
	if err != nil {
		t.Error(err)
		return
	...
	if err = BMW990C070Validate(buf); err != nil {
		t.Errorf("Invalid test binary file")
		return
	}
	...
	sum, _ := BMW173Checksum(buf, false)
	if sum != 0x56A3 {
		t.Errorf("Checksum failed, expected: %X got: %X", 0x56A3, sum)
	}
}
