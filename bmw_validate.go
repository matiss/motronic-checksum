...
// BMW157C165Validate validates ECU with code ending 157
func BMW157C165Validate(buf []byte) error {
	// Validate length
	if len(buf) != 0x8000 {
	...
	if buf[hwStart] != 0x37 || buf[hwStart+1] != 0x35 || buf[hwStart+2] != 0x31 {
	...
	if buf[reg2Start] != 0x02 || buf[reg2Start+1] != 0x25 || buf[reg2Start+2] != 0x59 || buf[reg2Start+3] != 0xFF {
...
// BMW590C597Validate validates ECU with code ending 590 and chip ending 597
...
// BMW950C167Validate validates ECU with code ending 950 and chip ending 167
func BMW950C167Validate(buf []byte) error {
	// Validate length
	if len(buf) != 0x8000 {
		return fmt.Errorf("Invalid file length")
	}
	...
	// Validate hardware number
	if buf[hwStart] != 0x30 || buf[hwStart+1] != 0x35 || buf[hwStart+2] != 0x39 {
		return fmt.Errorf("Invalid hardware number")
	}
